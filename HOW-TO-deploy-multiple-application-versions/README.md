...
Now that we have all of the components in place for managing two separate versions of Numbers in our environment, simply assigning a users workstation to the `Numbers` Static Computer Group will ensure that the proper version is installed. Additionally, `Numbers` will automaticall be upgraded when a users workstation upgrades to macOS Sierra.
